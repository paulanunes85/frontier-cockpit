...
Options:
  ...
  --no-build                 Start the container stack without rebuilding images.
...
# Container runtime detection: Docker is preferred when both are installed;
# Podman is a drop-in alternative. Contributed by @jkjunior.
...
# Compose tool selection. For Docker the native "docker compose" v2 plugin is
# preferred over the legacy standalone docker-compose. For Podman the
# podman-compose tool is preferred, falling back to "podman compose".
# The value is either a standalone binary name or the runtime name, in which
# case the stack is started with "<runtime> compose".
find_compose_tool() {
  local runtime="$1"
...
if [[ -z "$container_runtime" ]]; then
  # macOS keeps the Docker CLI inside Docker.app until the PATH is extended.
  if [[ "$(uname -s)" == "Darwin" ]] && [[ -x "/Applications/Docker.app/Contents/Resources/bin/docker" ]]; then
    export PATH="/Applications/Docker.app/Contents/Resources/bin:$PATH"
    container_runtime="docker"
  else
    fail "No container runtime was found. Install Docker Desktop, Docker Engine, or Podman."
  fi
fi
...
${fish_block_end}"
  ...
  for profile in "${profiles[@]}"; do
    ...
    elif ! grep -q "$block_start" "$profile"; then
      printf '\n%s\n' "$block" >> "$profile"
...
ensure_docker() {
  command -v "$CONTAINER_RUNTIME" >/dev/null 2>&1 || fail "$CONTAINER_RUNTIME CLI was not found. Install Docker Desktop, Docker Engine, or Podman."
  "$CONTAINER_RUNTIME" info >/dev/null 2>&1 || fail "$CONTAINER_RUNTIME is installed, but the daemon is not running. Start $CONTAINER_RUNTIME and rerun."
  ok "$CONTAINER_RUNTIME daemon is running (compose via $COMPOSE_TOOL)."
}
