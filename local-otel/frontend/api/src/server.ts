...
// Which population a number actually describes. The workspace selector in the
// UI only narrows metrics materialized per session; editor-level GenAI metrics
// carry no repository attribution, and the AI Credits pool is billed across all
// work. Declaring the scope keeps the dashboard from implying a filter that the
// underlying telemetry cannot honor.
...
// The manual Refresh control asks for fresh data; scheduled view switches reuse
// the short-lived cache.
export function freshFromUrl(url: URL): boolean {
  return url.searchParams.get("fresh") === "true";
}

// Cache keys must carry every input that changes the result, otherwise one
// workspace or plan selection could serve another's numbers.
export function summaryCacheKey(url: URL): string {
...
export function sessionsCacheKey(range: string, repo: string | null): string {
  return `${range}|${repo ?? "all"}`;
}

// One dashboard refresh calls summary, sessions, and coach, and coach and
// planner rebuild aggregates the other endpoints just computed. A short TTL
// collapses that duplication without ever showing data older than one
// materializer interval.
const summaryCache = new AsyncCache<Awaited<ReturnType<typeof summary>>>({
  ttlMs: numberFromEnv("CACHE_SUMMARY_TTL_MS", 15000)
});
const sessionsCache = new AsyncCache<Awaited<ReturnType<typeof sessionsBreakdown>>>({
  ttlMs: numberFromEnv("CACHE_SESSIONS_TTL_MS", 15000)
});
// The registry sidecar reseeds prices every 5 minutes, so anything shorter only
// adds load without adding freshness.
const modelPriceCache = new AsyncCache<Map<string, ModelPrice>>({
  ttlMs: numberFromEnv("CACHE_MODEL_PRICES_TTL_MS", 300000)
});

function cachedSummary(url: URL, fresh = freshFromUrl(url)) {
  return summaryCache.load(summaryCacheKey(url), () => summary(url), fresh);
}

function cachedSessions(range: string, repo: string | null, fresh = false) {
  return sessionsCache.load(sessionsCacheKey(range, repo), () => sessionsBreakdown(range, repo), fresh);
}

function cachedModelPrices(fresh = false) {
  return modelPriceCache.load("registry", () => modelPriceMap(), fresh);
}

// Session metrics are materialized per trace and carry repository attribution,
...
async function summary(url: URL) {
  ...
  const [
    ...
    workspaceBreakdown(range, repoLabelMatcher),
  ...
  return {
    ...
    dataBoundary: {
      localOnly: [
        ...
        "Tool results",
        "DuckDB files",
        "Grafana embedded SQLite metadata"
      ],
      safeForwarding: [
        "Sanitized metrics",
        "Sanitized traces",
...
async function coach(url: URL) {
  const fresh = freshFromUrl(url);
  const [data, sessions, modelPrices] = await Promise.all([
    cachedSummary(url, fresh),
    cachedSessions(rangeFromUrl(url), repoFromUrl(url), fresh),
    cachedModelPrices()
  ]);
  return {
    generatedAt: new Date().toISOString(),
...
const getRoutes: Record<string, RouteHandler> = {
  ...
  "/api/summary": async (url, response) => jsonResponse(response, 200, await cachedSummary(url)),
  "/api/sessions": async (url, response) =>
    jsonResponse(response, 200, await cachedSessions(rangeFromUrl(url), repoFromUrl(url), freshFromUrl(url))),
