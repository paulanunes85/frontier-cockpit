...
**Security note:** `.env` files often contain secrets (tokens, passwords). They are gitignored by default. Use `rhdh local backup` to archive them safely — the backup command warns about credential files.
