...
Edges are labeled with the relationship (HTTPS, token limit plus semantic cache, route, cache plus memory, retrieve, discover, govern, screen, authN, trace). Boundaries use the ms-identity accent colors; the boxes are neutral placeholders.
